...
import {
  Tag, Plus, X, Check, Copy,
  ToggleLeft, ToggleRight, Trash2,
  Percent, Calendar, Users,
} from "lucide-react";
...
function CreateModal({ onClose, onCreate }) {
  ...
  return (
    <main className="fixed inset-0 z-[500] flex items-center justify-center p-4" onClick={(e) => e.target === e.currentTarget && onClose()}>
      ...
      <div className="relative bg-[#111112] border border-[#D4AF37]/20 w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
        ...
        <div className="p-5 sm:p-6 space-y-4 overflow-y-auto flex-1">
          ...
          {/* Code */}
          <div>
            <label className="text-[10px] text-gray-500 font-black uppercase tracking-widest mb-2 block">Code promo *</label>
          ...
          {/* Type de réduction */}
          <div>
            <label className="text-[10px] text-gray-500 font-black uppercase tracking-widest mb-2 block">Type de réduction *</label>
            <FloatSelect
              label="Type"
              value={form.type}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div>
              <label className="text-[10px] text-gray-500 font-black uppercase tracking-widest mb-2 block">Commande min (FCFA)</label>
              <FloatInput
                label="Ex: 50000"
                name="minOrder"
                type="number"
          ...
          {form.code && form.discount && (
            <div className="p-3.5 bg-[#D4AF37]/8 border border-[#D4AF37]/20 rounded-xl">
              <p className="text-[10px] text-gray-500 font-black uppercase mb-1">Aperçu</p>
              <p className="text-sm font-black text-white">
                Code <span className="text-[#D4AF37] font-mono tracking-widest">{form.code}</span>
                {" → "}
                {form.type === "PERCENT" ? `-${form.discount}%` : `-${form.discount} FCFA`}
          ...
          {error && (
            <div className="p-3 bg-red-500/10 border border-red-500/20 rounded-xl text-red-400 text-xs font-bold text-center">{error}</div>
          )}
        </div>

        {/* Footer */}
        <div className="px-5 sm:px-6 py-4 border-t border-white/5 flex-shrink-0">
          <button onClick={submit} className="w-full py-3.5 bg-[#D4AF37] text-black font-black uppercase text-[11px] tracking-widest rounded-xl hover:opacity-90 transition-opacity">
            Créer le code promo
          </button>
        </div>
      </div>
...
export default function ManagePromos() {
  ...
  const addPromo = (newPromo) =>
    setDb((prev) => ({ ...prev, promoCodes: [newPromo, ...(prev.promoCodes || [])] }));

  const toggleActive = (id) =>
    setDb((prev) => ({
      ...prev,
      promoCodes: prev.promoCodes.map((p) => p.id === id ? { ...p, active: !p.active } : p),
    }));
  ...
  const deletePromo = async (id, code) => {
    ...
    if (!ok) return;
    setDb((prev) => ({ ...prev, promoCodes: prev.promoCodes.filter((p) => p.id !== id) }));
  };
  ...
  const getStatus = (p) => {
    if (!p.active)                                        return { label: "Inactif", color: "text-gray-400   bg-white/5       border-white/10" };
    if (p.expiresAt && new Date(p.expiresAt) < new Date()) return { label: "Expiré",  color: "text-red-400    bg-red-500/10    border-red-500/20" };
    if (p.maxUses && p.uses >= p.maxUses)                  return { label: "Épuisé",  color: "text-orange-400 bg-orange-500/10 border-orange-500/20" };
    return { label: "Actif", color: "text-green-400 bg-green-500/10 border-green-500/20" };
  };
    ...
    <main className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div className="grid grid-cols-3 gap-3 flex-1 min-w-0">
          {[
            ...
            <div key={label} className="bg-[#111112] border border-white/5 rounded-2xl p-3 sm:p-4">
              <div className="text-xl sm:text-2xl font-black" style={{ color }}>{count}</div>
              <div className="text-[9px] sm:text-[10px] font-bold text-gray-500 uppercase tracking-wider mt-0.5 leading-tight">{label}</div>
            </div>
          ))}
        </div>
        <button
        ...
        >
          <Plus size={15} /> <span className="hidden sm:inline">Nouveau code</span><span className="sm:hidden">Nouveau</span>
        </button>
      </div>
      ...
      {promos.length === 0 ? (
        <div className="bg-[#111112] border border-white/5 rounded-2xl py-16 text-center">
          ...
          <p className="text-gray-500 text-sm font-bold">Aucun code promo créé</p>
          <button
            onClick={() => setShowModal(true)}
            ...
            return (
              <div
                ...
                className={`bg-[#111112] border rounded-2xl px-4 sm:px-5 py-4 flex items-center justify-between gap-3 flex-wrap transition-colors ${
                  p.active ? "border-white/5 hover:border-white/10" : "border-white/3 opacity-60"
                }`}
              >
                {/* Gauche */}
                <div className="flex items-center gap-3 sm:gap-4 min-w-0 flex-1">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex items-center gap-2 flex-wrap mb-0.5">
                      <span className="font-mono font-black text-white text-sm sm:text-base tracking-widest">{p.code}</span>
                      <button onClick={() => copyCode(p.code)} className="p-1 rounded text-gray-600 hover:text-[#D4AF37] transition-colors" title="Copier">
                        {copied === p.code ? <Check size={12} className="text-green-400" /> : <Copy size={12} />}
                      </button>
                      <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded border ${status.color}`}>{status.label}</span>
                    </div>
                    {/* Infos */}
                    <div className="text-[10px] sm:text-[11px] text-gray-500 flex items-center gap-2 sm:gap-3 flex-wrap">
                      <span className="text-[#D4AF37] font-bold">
                        {p.type === "PERCENT" ? `-${p.discount}%` : `-${p.discount} FCFA`}
                      </span>
                      {p.minOrder > 0 && <span>Min {p.minOrder.toLocaleString()} F</span>}
                      {p.maxUses
                        ? <span>{p.uses}/{p.maxUses} util.</span>
                        : <span>{p.uses} util.</span>
                      }
                      {p.expiresAt && <span className="hidden sm:inline">Exp. {new Date(p.expiresAt).toLocaleDateString("fr-FR")}</span>}
                    </div>
                  </div>
                </div>
      ...
      {showModal && <CreateModal onClose={() => setShowModal(false)} onCreate={addPromo} />}
    </main>
  );
}