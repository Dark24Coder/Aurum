...
function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    if (result.success) {
      navigate(result.role === "ADMIN" ? "/dashboard/admin" : "/dashboard/user");
    } else {
      ...
      <div className="w-full max-w-md bg-[#161617] border border-white/5 rounded-[2rem] p-7 sm:p-10 shadow-2xl animate-fade-in-up">
        ...
        {step === "LOGIN" && (
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            />
            <Button type="submit" variant="gold" size="full" loading={authLoading}>
              Entrer dans l'espace
            </Button>
            <div className="flex flex-col items-center gap-3 mt-4">
              <Button variant="ghost" size="sm" onClick={() => setStep("FORGOT")}>
                Identifiants perdus ?
              </Button>
              <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">
                Nouveau ici ?{" "}
                <Link to="/register" className="text-[#D4AF37] hover:underline ml-1">
                  Créer un compte
                </Link>
              </p>
            </div>
        ...
        {step === "FORGOT" && !emailSent && (
          <form onSubmit={handleForgot} className="space-y-4">
            <FloatInput label="Email de récupération" name="email" type="email" value={form.email} onChange={handleChange} required icon={Mail} />
            <Button type="submit" variant="gold" size="full" loading={authLoading}>
              Récupérer l'accès
            </Button>
          </form>
